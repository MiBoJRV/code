...
export const Button: React.FC<ButtonProps> = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'group relative max-w-fit pl-[20px] pr-[5px] mr-[15px] flex items-center bg-no-repeat bg-cover bg-cutted-btn hover:bg-cutted-hover-btn transition-all',
        btnStyles,
        className
      )}
    >
      {isLeftIcon && icon && <div className="mr-2.5">{icon}</div>}
      {title && (
        <ButtonText title={title} className={cn('text-grey', btnTextStyles)} />
      )}
      {isRightIcon && icon && <div className="ml-2.5">{icon}</div>}
      <div
        className={cn(
          'absolute w-[15px] right-[-15px] bg-no-repeat bg-cover bg-right bg-cutted-btn group-hover:bg-cutted-hover-btn transition-all',
          btnStyles
        )}
      />
    </button>
